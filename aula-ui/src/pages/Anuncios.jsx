...
const NEWS = [
  ...
  },
  {
    ...
    to: "/detalle-aviso", // si quieres que abra el detalle
  },
  {
    id: "t1",
    icon: "campaign",
  ...
  return (
      ...
      <main className="flex-1 pt-16">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-8 lg:px-12 xl:px-14 2xl:px-16 py-6">
          {/* Hero del curso (misma estética) */}
          <section
            className="bg-primary-DEFAULT text-white rounded-lg p-6 md:p-8 mb-8 shadow-lg min-h-40 md:min-h-48 flex items-center"
            style={{
              background:
                "linear-gradient(rgba(0, 51, 102, 0.85), rgba(0, 51, 102, 0.85)), url('https://i.imgur.com/d9T3Y8y.jpeg') no-repeat center center / cover",
            }}
          >
          ...
          {/* Tabs del curso (negro por defecto, azul al hover) */}
          ...
          {/* Filtros (match estética de botones del otro layout) */}
          <div className="flex flex-wrap gap-2 mb-6">
            {FILTERS.map((f) => (
              <button
                ...
                className={[
                  "px-3 py-1.5 rounded-full border border-border-light dark:border-border-dark text-sm hover:bg-background-light dark:hover:bg-background-dark transition-colors",
                  active === f.key ? "ring-2 ring-primary-DEFAULT" : "",
                ].join(" ")}
              >
                {f.label}
              </button>
          ...
          {/* Lista de avisos (cards como en las demás vistas) */}
          <div className="space-y-4">
            {items.map((n) => {
              ...
              const classes =
                "bg-card-light dark:bg-card-dark p-4 rounded-lg border border-border-light dark:border-border-dark shadow-sm hover:shadow-md transition-shadow";
            ...
            {items.length === 0 && (
              <p className="text-sm text-subtle-light dark:text-subtle-dark">No hay avisos para este filtro.</p>
            )}
          </div>
        </div>
      </main>
